...
> **2026-09-24 作者已裁定并执行**：3.1 当场修掉；3.2 走「删 Python 那份、接上 `hooks.ts`」；
> 3.3 先在真书开开关试跑——试跑发现真正卡的不是开关而是**判据与伏笔表格式不符**，已一并修复。
> 执行过程、实测证据与残留缺口见 **`docs/23-钩子与伏笔判据处置.md`**。以下三节保留当时的判断原文。
...
### 3.2 `hook_check` 有两份实现，两份都不生效（未修，需决定）

**2026-09-24 补充更正**：本节原先给的 A/B 两个选项（改读按章细纲／跨语言复用
`readiness.ts`）**基于不完整侦察**——当时只追了 Python 侧，漏看了 TS 侧。
实际是**同一个判据有两份副本，且两份都不生效**：

| 实现 | 位置 | 规模 | 不生效的原因 |
|---|---|---|---|
| Python `_hook_check` | `gates/consistency_check.py:194` | 约 30 行 | `import brief` 取细纲钩子，而 `brief.py` **全仓不存在**；`except Exception: return` 把 `ImportError` 一并吞掉 |
| TS `hooks.ts` | `packages/core/src/hooks.ts` | 213 行 + 10 项测试 | 已从 `core` 导出，但**零调用者**（除自身、`index.ts` re-export、`hooks.test.ts`） |

后者的成熟度**高于**前者，且不是草稿——`docs/ne-架构与契约.md §7.2`
专门为它立了一节「边界比功能重要」，纪律是实测 34 章人工对账后定的：

- 红灯只当**人工复核入口**，不当结论；
- **不接进 CI 硬失败**（会把「作者有意改写」误杀成「质量缺陷」）；
- 锚词只取引号内内容、剔除人名（实测：把人名当锚词 → 必然误报，红灯沦为噪音）；
- 匹配用**片段**而非整串全等（实测 ch15「都/都得」一字之差误报）；
- 语义判定只能靠人工读或 LLM，不要伪装成词面校验。

因此原先那两个选项都不成立：它们都等于**在 Python 侧再养一份**，
正是本项目反复禁止的「同一判据两份副本」。修正后的取向：

- **A（推荐）**：删掉 Python `_hook_check` 与 `checks.hook_check` 键，
  把 `hooks.ts` 接上（`readHookSpecs` + `auditHooks` 现成）。
  接入时必须守住 §7.2 的纪律——**只出只读报告，不做成拦截级 finding**。
- **B**：两份都删，把「钩子有没有落到位」完全交给 `/review` 人工评审
  （`content/workflows/chapter-review.md` 第五问已是「钩子落在最后哪几句？与细纲的钩子一致吗？」）。
  理由：`hooks.ts` 自己就写着「词面匹配本质上不可靠」，一个只报线索的东西是否值一个命令，可议。
- **C**：保持现状（两份都留，都不生效）。
...
**2026-09-24 补充**：这条对**新书**成立，但对**两本真书不成立**——实测两本都
`paths.foreshadow` 已声明、伏笔表文件已存在（高武 3371 字节、仙侠 3803 字节）：

| 书 | `paths.foreshadow` | 文件 | `checks.foreshadow_check` |
|---|---|---|---|
| 高武 | `outline/伏笔回收总表.md` | 存在 | `null`（未启用） |
| 仙侠 | `outline/伏笔回收总表.md` | 存在 | `null`（未启用） |

即真书的阻塞点**只是开关没开**，打开即可生效，不需补任何代码或文件；
新书的阻塞点才是缺键缺文件。这两类要分开处置，别当成一件事。
