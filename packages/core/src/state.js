...
/** 严重度权重：取该章最大值当 worst */
const SEVERITY_WEIGHT = { 严重: 4, 中等: 3, 轻微: 2, 提示: 1 };
...
async function loadFileRegex(bookRoot) {
    try {
...
async function rebuildState(bookRoot) {
 ...
 */
async function sweepStaleGateStatus(root, chapters) {
    await Promise.all(chapters.map(async (ch) => {
        if (ch.gateStatus === null)
            return;
    ...
    if (opts.force !== true) {
        ...
        if (raw !== null) {
            try {
                ...
                if (parsed.schemaVersion === 1 && parsed.bookRoot === root)
                    state = parsed;
 ...
 */
export function summarizeGateResult(result) {
    const map = new Map();
